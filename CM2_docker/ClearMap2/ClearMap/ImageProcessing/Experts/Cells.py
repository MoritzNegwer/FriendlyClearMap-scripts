...
default_cell_detection_parameter = dict( 
  ...
  #Method. Standard = Clearmap2 built-in. Specify here if Ilastik should be used. 
  cell_detection_method = 'Clearmap_2_built_in',
)
  ...
  * Each step also has an additional parameter 'save' that enables saving of 
  ...
  for key in cell_detection_parameter.keys():
    ...
    if isinstance(par, dict):
      ...
      if filename:
        ap.initialize_sink(filename, shape=shape, order=order, dtype='uint32');
        
  cell_detection_parameter.update(verbose=processing_parameter.get('verbose', False));
  
  # Check method, (built-in or Ilastik)
  cell_detection_method = cell_detection_parameter.get('cell_detection_method',None)
  if cell_detection_method == 'Clearmap_2_built_in':
      #If CM2 built-in method: run blockwise detection
  ...
  elif cell_detection_method == 'Ilastik':
    #detect cells using Ilastik codepath adapted from CM1
    
    #import old CM1 Ilastik code path
    from ClearMap.ImageProcessing.IlastikCellFinder import IlastikCellFinder, IlastikClassification, StackProcessing
    
    
    #check if Ilastik can be initialized, if not throw error 
    if IlastikCellFinder.Initialized:
      #define Ilastik cell classification paramters
      detectCells = IlastikClassification.classifyCells;
  
      #nasty hack to bring source format into CM1 format
      source_ilastik = (source.file_list[0].replace("0000","\d{4}"))
      sink_ilastik = (sink, cell_detection_parameter['findCellIntensityParameter']['intensities_sink'])
      
      #process
      StackProcessing.sequentiallyProcessStack(source_ilastik, sink = sink_ilastik, function = detectCells, verbose = True, 
                                         chunkSizeMax = processing_parameter['size_max'],
                                         chunkSizeMin = processing_parameter['size_min'],
                                         chunkOverlap = processing_parameter['overlap'],
                                         x = all,
                                         y = all,
                                         z = all,
                                         classifyCellsParameter = cell_detection_parameter['classifyCellsParameter'],
                                         detectCellsParameter = cell_detection_parameter['findCellIntensityParameter'],
                                         processingParameter = processing_parameter);  
      
      #debug: Save results 
      #np.save("/home/wirrbel/2020-12-15_CM2_test/Test_imgs_VIP_m3/debug_results.npy", results)
      #np.save("/home/wirrbel/2020-12-15_CM2_test/Test_imgs_VIP_m3/debug_intensities.npy", results)
      
      #debug
      #return results 
      
      #load results 
      results = np.load(sink)
      
      #load intensities 
      intensities = np.load(cell_detection_parameter['findCellIntensityParameter']['intensities_sink'])
      intensities = intensities[:,1]
      
      #convert results to CM2 format
      cells = np.zeros((len(results),),dtype=[('x', int),('y',int),('z',int),(cell_detection_parameter['intensity_detection']['measure'][0],float)])
      ...
      #add intensities
      cells[cell_detection_parameter['intensity_detection']['measure']] = intensities 
