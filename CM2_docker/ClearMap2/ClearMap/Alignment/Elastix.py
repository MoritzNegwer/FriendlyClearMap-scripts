...
def align(fixed_image, moving_image, affine_parameter_file, bspline_parameter_file = None, result_directory = None, processes = None, moving_points = None, fixed_points = None):
  ...
  if bspline_parameter_file is None:
    ...
    cmd = '%s -threads %d -m %s -f %s -p %s -p %s -out %s -mp %s -fp %s' % (elastix_binary, processes, moving_image, fixed_image, affine_parameter_file, bspline_parameter_file, result_directory, moving_points, fixed_points);
    #mp and fp added by Moritz 2020-10-20
...
def transform_points(source, sink = None, transform_parameter_file = None, transform_directory = None, indices = False, result_directory = None, temp_file = None, binary = True):
  ...
  #read data / file 
  #debug 2020-12-16
  print ("elastix transform points, sink = ",sink)
  #if (sink == []) or (sink == None): # return sink as file name
