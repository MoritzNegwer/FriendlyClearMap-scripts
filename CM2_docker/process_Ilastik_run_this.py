...
import sys
sys.path.append('/CloudMap/ClearMap2/')
...
if __name__ == "__main__":
  ...
  directory = '/CloudMap/Data/' #must end on trailing slash   
  
  expression_raw      = '200127_Lukas_2018-08-08_m1_tdtom_568_18-10-37/18-10-37_Lukas_2018-08-08_m1_tdtom_568_UltraII_C00_xyz-Table Z <Z,4>.ome.tif'           
  ...
  #Action = 'Find_Cells_With_Ilastik' #find cells with Ilastik. If so, you will need to specify the Ilastik process file below 
  #Ilastik_Classifier = "/CloudMap/Data/2021-02-23_Lukas_PV_P50_reanalysis.ilp"
  
  #Action = 'Import_cells_csv' #Import external cell counts from e.g. Arivis (needs to be csv with only x/y/z coordinates, no header. You'll need to specify the file below. 
  #Cells_csv = "/CloudMap/Data/cells.csv" 

  #Action = 'ClearMap_2_detection' #Choose this if you want to run the standard ClearMap2 cell detection. Still to be implemented. 
  ...
  atlas_annotation_file = io.join(atlas_dir,'annotation_25_full.nrrd')
  ...
  #Stop here if Action = Preprocess
  ...
  #Check if required action is either finding cells with Ilastik, regular Clearmap, or importing previously found cell coordinates
  if Action == 'Import_cells_csv': 
    #Import external cell counts from e.g. Arivis (needs to be csv with only x/y/z coordinates, comma separated, no header. Optional 4th column with intensities) 
    #CSV file defined in Cells_csv variable at the top of the file. 
    cells_array = io.read(Cells_csv)

    #if no intensities, add additional column with all being value 1
    if cells_array.shape[1] < 4:
      cells_array = np.hstack((cells_array, np.ones((cells_array.shape[0], 1), dtype=cells_array.dtype)))

    #write to npy file
    io.write(sink=ws.filename('cells', postfix='raw'),data = cells_array)

  elif Action == 'Find_Cells_With_Ilastik': #find cells with Ilastik. Ilastik Classifier has been specified at the top of the file. 
	  #%% Cell detection:
  ...
  #Alternatively, if you want to run the ClearMap detection 
  elif Action == 'ClearMap_2_detection':
     	#still to be troubleshot, this is straight from the original CM2 script
	cell_detection_parameter = cells.default_cell_detection_parameter.copy();
	cell_detection_parameter['illumination_correction'] = None;
	cell_detection_parameter['background_correction'] = None;
	cell_detection_parameter['intensity_detection']['measure'] = ['source'];
  ...
  custom_label_csv = pandas.read_csv('/CloudMap/Data/atlas/P56_CCF2/regions_IDs.csv')
