...
#re-import CM1 io 
import ClearMap.ImageProcessing.IlastikCellFinder.CM1_IO as io

from ClearMap.ImageProcessing.IlastikCellFinder.ParameterTools import writeParameter
from ClearMap.ImageProcessing.IlastikCellFinder.ProcessWriter import ProcessWriter;
...
def calculateSubStacks(source, z = all, x = all, y = all, **args):
    ...
    #determine z ranges
    fs = io.dataSize(source); #CM1 code
    #debug 
    #print("source = ",source)
    #fs = source.shape
    ...
    #zr = tuple for z range
    zr = io.toDataRange(zs, r = z); #original CM1 code
    #zr = [zs, zs] 
...
def sequentiallyProcessStack(source, x = all, y = all, z = all, sink = None,
    ...
    #debug 
    print ("sequentiallyProcessStack here .source = ", source)
    
    #determine z ranges  
    
    subStacks = calculateSubStacks(source, x = x, y = y, z = z, 
                                   processes = 1, chunkSizeMax = chunkSizeMax, chunkSizeMin = chunkSizeMin, chunkOverlap = chunkOverlap,  
                                   chunkOptimization = True, verbose = verbose);
    
    nSubStacks = len(subStacks);
    print ("nSubStacks = ",nSubStacks);    
