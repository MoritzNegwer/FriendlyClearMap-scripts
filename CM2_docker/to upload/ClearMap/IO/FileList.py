...
def convert(source, sink, processes = None, verbose = False,experimentalFixShape = False):
  ...
  #test: try to fix weird 4th dimension in sources from LSM 
  if experimentalFixShape == False:
      shape = source.shape; #standard 
  else:
      shape = source.shape[:3] #only take first 3 dimensions
  ...
  #debug
  print("io.create imminent, printing shape: ", shape)
  #end debug
