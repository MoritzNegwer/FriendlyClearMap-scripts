...
def resampleData(source, sink = None,  orientation = None, dataSizeSink = None, resolutionSource = (4.0625, 4.0625, 3), resolutionSink = (25, 25, 25), 
                 processingDirectory = None, processes = 1, cleanup = True, verbose = False, interpolation = 'linear', **args):
    ...
    #troubleshoot 2020-12-09 to get around concurrent lock on stdout
    ...
    #with concurrent.futures.ProcessPoolExecutor(max_workers=11,mp_context=multiprocessing.get_context('fork')) as executor:
    #    executor.map(_resampleXYParallel, argdata)
    
    print ("all pools have concluded, waiting for closing")
    #pool.join()
    ...
    #testing whether chunksize makes a difference
    #chunksize = 10
    #pool.imap_unordered(_resampleXYParallel, argdata);
