...
class LabelInfo(object):
    ...
    def initialize(slf, annotationFile = DefaultAnnotationFile):
        ...
        with open(annotationFile) as dfile:
            ...
            #skip header
            next(reader)
            print ("reading annotation file CSV")
            ...
            print ("finished annotation file CSV reading")
            #print(labels)
            dfile.close();
    ...
    def toLabelAtCollapse(slf, iid):
        ...
        #    return i;
        #else:
	#notes KeyError if some value is not in the region list, 
	#Crucually: Does not throw error, but just ignores value! 
        try:
            #print "toLabelAtCollapse: ", i
            return slf.collapseMap[i];
        except KeyError:
            print("return skipped due to KeyError")
            pass


#Moved into CountPointsInRegion function below
#Label = LabelInfo();
...
def labelPoints(points, labeledImage = DefaultLabeledImageFile, level = None, collapse = None):
 ...
 #annotationFile added by Mo 2020-10-07
def countPointsInRegions(points, labeledImage = DefaultLabeledImageFile, annotations = DefaultAnnotationFile, intensities = None, intensityRow = 0, level= None, allIds = False, sort = True, returnIds = True, returnCounts = False, collapse = None):
    #added by Mo, moved down from line 172 
    #added annotationFile call 
    #print( "Creating Label")

    
    #make global Label if it doesn't exist yet 
    #if not 'Label' in globals:
    global Label
    Label = LabelInfo(annotations);    
    
    points = io.readPoints(points);
    intensities = io.readPoints(intensities);
    
    #annotationFile added by Mo 2020-10-07
    #print( "generating point labels")
    ...
    if returnIds:
        if cci is None:
            #print( "returnIDs is true but cci = None, returning ll + cc")
            return ll,cc
            #return ll, cc
            #debug 2020-10-07 
        else:
            if returnCounts:
                ...
                #print ("returnIDs is true, thus returning ll + cci")
                return ll, cci
    else:
        if cci is None:
            return cc;
...
def labelToName(label):
    ...
    '''
    #troubleshooting 2020-12-03
    #to get around UnicodeEncodeError
    ...
    for x in label:
        try:
            xlist.append(Label.name(x))
        except UnicodeEncodeError:
            print("error found at label = ", x)
            #declare as background
            xlist.append(Label.name(0))
